...
namespace Catch {
    ...
    friend bool operator == ( const T& lhs, Approx const& rhs ) {
        ...
        bool relativeOK = std::fabs(lhs_v - rhs.m_value) < rhs.m_epsilon * (rhs.m_scale + (std::max)(std::fabs(lhs_v),
    ...
    friend bool operator == ( double lhs, Approx const& rhs ) {
        // Thanks to Richard Harris for his help refining this formula
        bool relativeOK = std::fabs( lhs - rhs.m_value ) < rhs.m_epsilon * (rhs.m_scale + (std::max)( std::fabs(lhs),
                std::fabs(rhs.m_value) ) );
        if (relativeOK) {
            return true;
        }
        ...
        Class cls = classes[c];
        {
            ...
            for( u_int m = 0; m < count ; m++ ) {
                ...
                if( startsWith( methodName, "Catch_TestCase_" ) ) {
                    ...
                    getMutableRegistryHub().registerTest( makeTestCase( new OcMethod( cls, selector ), className, name.c_str(),
...
namespace Catch {
    ...
    Win32ColourImpl() : stdoutHandle( GetStdHandle(STD_OUTPUT_HANDLE) ) {
        ...
        originalForegroundAttributes = csbiInfo.wAttributes & ~( BACKGROUND_GREEN | BACKGROUND_RED | BACKGROUND_BLUE |
